...
import {ExpenseFormData, expenseSchema} from "@/lib/schemas";
...
export default function ExpenseForm({ expense, onClose }: ExpenseFormProps) {
    ...
    // Fetch authenticated user to auto-fill agent name
    ...
    const {
        ...
        defaultValues: {
            ...
            agentName: expense?.agentName || agentNameFromUser,
            ...
            paymentAccountType: expense?.bankAccountId
                ? 'BANK'
    ...
    const onSubmit = async (data: any, isDraft: boolean) => {
        try {
            const dataToSubmit = {
                ...data,
                agentName: agentNameFromUser, // Always use logged-in user
                expenseStatus: isDraft ? ExpenseStatus.DRAFT : ExpenseStatus.PENDING,
                bankAccountId:
                    data.paymentAccountType === 'BANK' ? data.bankAccountId : undefined,
                cashAccountId:
                    data.paymentAccountType === 'CASH' ? data.cashAccountId : undefined,
            };
                ...
                >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            />
                            {errors.expenseName && (
                                <p className="mt-1 text-sm text-red-600">{errors.expenseName.message}</p>
                            )}
                        </div>
                                                    ...
                                                    {selectedAccount && (
                                                        <p
                                                            className={`mt-1 text-sm ${
                                                                isLowBalance ? 'text-red-600' : 'text-gray-500'
                                                            }`}
                                                        >
                    ...
                    <Card className="p-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                            {/* Read-only Agent Name Display */}
                            <div className="flex items-center gap-3">
                                <Label className="min-w-[100px]">Agent Name</Label>
                                <div className="flex items-center gap-2 text-sm text-gray-700">
                                    <User className="w-4 h-4 text-gray-400" />
                                    <span>{agentNameFromUser}</span>
                                </div>
                            </div>
                    ...
                    <Card className="p-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                            ...
                            <div>
                                <Label htmlFor="paymentModeDescription">
                                    Payment Mode Description (Optional)
                                </Label>
                                <Controller
                    ...
                    <div className="flex justify-end space-x-4">
                        ...
                        >
                            {isCreating && watch('expenseStatus') === ExpenseStatus.DRAFT
                                ? 'Saving...'
                                : 'Save as Draft'}
                        </Button>
                        ...
                        {!isStaff && (
                            <Button
                                ...
                                disabled={isCreating || isUpdating || Object.keys(errors).length > 0}
                            >
                                {isCreating && watch('expenseStatus') === ExpenseStatus.PENDING
                                    ? 'Submitting...'
                                    : isEditing
                                        ? 'Update'
                                        : 'Create'}
