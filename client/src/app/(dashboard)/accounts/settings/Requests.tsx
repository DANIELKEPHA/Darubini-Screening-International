...
import LeaveRequestModal from '@/app/(dashboard)/accounts/settings/components/leave/LeaveRequestModal';

const MyLeaveRequests = () => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [filterStatus, setFilterStatus] = useState('ALL');

  const { data: requestsData, isLoading: requestsLoading, error: requestsError } = useGetMyLeaveRequestsQuery({
    status: filterStatus === 'ALL' ? undefined : filterStatus,
  });

  const { data: balance, isLoading: balanceLoading, error: balanceError } = useGetLeaveBalanceQuery();

  const [downloadLeavePdf, { isLoading: isDownloading }] = useDownloadLeaveApprovalPdfMutation();
  ...
  const handleDownloadPdf = async (leaveRequestId: number | string) => {
    try {
      await downloadLeavePdf(leaveRequestId.toString()).unwrap();
    } catch (error: any) {
  ...
  const balanceItems = [
    { label: 'Annual Leave', entitled: Number(balance?.annualEntitled ?? 0), used: Number(balance?.annualUsed ?? 0), remaining: Number(balance?.annualRemaining ?? 0), color: 'blue' },
    { label: 'Sick Leave', entitled: Number(balance?.sickEntitled ?? 0), used: Number(balance?.sickUsed ?? 0), remaining: Number(balance?.sickRemaining ?? 0), color: 'emerald' },
    { label: 'Emergency Leave', entitled: Number(balance?.emergencyEntitled ?? 0), used: Number(balance?.emergencyUsed ?? 0), remaining: Number(balance?.emergencyRemaining ?? 0), color: 'orange' },
    { label: 'Maternity Leave', entitled: Number(balance?.maternityEntitled ?? 0), used: Number(balance?.maternityUsed ?? 0), remaining: Number(balance?.maternityRemaining ?? 0), color: 'pink' },
    { label: 'Paternity Leave', entitled: Number(balance?.paternityEntitled ?? 0), used: Number(balance?.paternityUsed ?? 0), remaining: Number(balance?.paternityRemaining ?? 0), color: 'purple' },
    { label: 'Compassionate Leave', entitled: Number(balance?.compassionateEntitled ?? 0), used: Number(balance?.compassionateUsed ?? 0), remaining: Number(balance?.compassionateRemaining ?? 0), color: 'indigo' },
  ];
  ...
  const getColorClass = (color: string, type: 'text' | 'bg' = 'text') => {
    ...
    return type === 'text' ? colors[color]?.text : colors[color]?.bg;
  };
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {balanceLoading ? (
              Array.from({ length: 6 }).map((_, index) => (
                  <div key={index} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm animate-pulse h-[172px]" />
              ))
          ) : (
              balanceItems.map((item) => (
                  <div key={item.label} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-all flex flex-col">
                    <h3 className="font-semibold text-gray-800 text-[15px] mb-3">{item.label}</h3>
                    ...
                    <div className="mt-4">
                      <div className="w-full bg-gray-200 rounded-full h-1.5">
                        <div
                            ...
                            style={{ width: item.entitled > 0 ? `${Math.min((item.used / item.entitled) * 100, 100)}%` : '0%' }}
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                      <div className="flex items-center gap-3">
                                    <span className={`inline-block px-4 py-1.5 rounded-full text-sm font-semibold border ${getStatusColor(req.status)}`}>
                                        {req.status}
                                    </span>
                        <div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {/* Download PDF Button - Only for APPROVED requests */}
                        {req.status === 'APPROVED' && (
                            <button
                                onClick={() => handleDownloadPdf(req.id)}
                                disabled={isDownloading}
                                className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-400 text-white text-sm font-medium rounded-xl transition-all"
                            >
                    ...
                    {req.approvals?.length > 0 && req.approvals[req.approvals.length - 1]?.comments && (
                        <div className="mt-5 p-5 bg-blue-50 rounded-xl border border-blue-100">
                          ...
                          <p className="text-gray-700">
                            {req.approvals[req.approvals.length - 1].comments}
                          </p>
                        </div>
                    )}
                  </div>
