...
const StaffSettings = () => {
    ...
    const onSubmit = async (data: SettingsFormData) => {
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/staff/${authUser.cognitoInfo.userId}`, {
                method: "PUT",
                headers: {
    ...
    if (!authUser || authUser.userRole !== "staff") {
            ...
            >
                ...
                <motion.div variants={itemVariants} className="mb-8">
                  ...
                  <span className="text-2xl font-bold text-white">
                    {authUser.userInfo?.name?.charAt(0) || "S"}
                  </span>
                                </div>
                                <div>
                                    <h2 className="text-xl font-bold text-gray-900">{authUser.userInfo?.name || "Staff User"}</h2>
                                    <div className="flex items-center gap-2 mt-1">
                    <span className="px-3 py-1 bg-blue-100 text-blue-700 text-xs font-medium rounded-full">
                      {authUser.userRole || "Staff"}
                    </span>
